...
확정한다. 0.65M 모델(flatten-dilated-bound 2.346 nm) 대비 328배 파라미터로 −83% —
...
| level1_cnn/flatten-dilated-bound (Task 5) | 0.66M | 2.346 | 1.83 / 3.28 / 2.34 / 1.93 |
...
- 학습 곡선 마일스톤 (train.log): ep5 4.31 (**baseline 4.599를 5에폭에 추월**),
  ep15 1.70 (**CNN 2.346을 ~14에폭에 추월**), ep16~19 1.8~2.2로 출렁인 뒤
...
3. **스케일 격차의 자리매김**: 0.66M → 213.2M (328×)로 MAE 2.346 → 0.3955 (−83%).
...
- 세션 유실 대비 인프라가 실전 검증됨: 세션 5회에 걸쳐 resume (미러 간격 5에폭 격자
  15/50/70/95에서 재개). 이 실험이 드러낸 인프라 이슈 3건과 수정 — GPU resume 시
