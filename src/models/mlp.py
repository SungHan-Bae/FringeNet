...
기본 블록은 ``Linear -> Norm -> Activation -> Dropout`` 순서이고, 마지막에 Linear head를
붙인다. 입력 표준화는 하지 않는다 — 첫 블록의 norm 층이 그 역할을 겸한다.
"""
...
_NORMS: dict[str, type[nn.Module] | None] = {
...
class MLPBlock(nn.Module):
    """MLP 기본 블록: Linear -> Norm -> Activation -> Dropout (+ 선택적 residual).

    residual=True면 블록 입력을 출력에 더한다. 입출력 폭이 다르면 bias 없는
    Linear projection으로 폭을 맞춘 skip을 쓰고, 같으면 identity skip이다.

    Shapes:
        forward: x (B, d_in) float -> (B, d_out) float
    """
    ...
    def __init__(
        ...
        layers: list[nn.Module] = [nn.Linear(d_in, d_out)]
        if norm_cls is not None:
            layers.append(norm_cls(d_out))
        ...
        if residual:
            self.skip = nn.Identity() if d_in == d_out else nn.Linear(d_in, d_out, bias=False)
...
class MLP(nn.Module):
    ...
    Args:
        ...
        hidden_dims: 은닉 블록 폭. 빈 시퀀스면 선형 회귀로 퇴화한다.
        activation: 블록 활성화 — "relu" | "gelu" | "silu".
        norm: 블록 정규화 — "batchnorm" | "layernorm" | "none".
        dropout: 각 블록 끝 dropout 확률. 0이면 층 자체를 넣지 않는다.
        residual: True면 블록마다 skip connection을 더한다
            (폭이 다르면 Linear projection, 같으면 identity).
    ...
    def __init__(
        ...
        if norm not in _NORMS:
            raise ValueError(f"norm은 {sorted(_NORMS)} 중 하나여야 한다 (받은 값: {norm!r})")
        ...
        for width in hidden_dims:
            layers.append(MLPBlock(width_in, width, activation, norm, dropout, residual))
            width_in = width
