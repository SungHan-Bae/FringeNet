...
**주의**: 반환된 채널별 λ에는 ~0.44 nm의 추정 잡음이 있다 (다항 차수를 올려도 줄지
않으므로 구조가 아니라 잡음이다). 그대로 고정하면 그 잡음만으로 R 오차 rms 0.0047이
생기므로, 본 피팅은 이 결과에 매끈한 3계수 곡선을 적합해 쓴다
(`calibrate.fit_lam_coefficients`).
"""
