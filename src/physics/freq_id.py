...
Stage A의 첫 단계다 — 본 피팅(`src/calibrate.py`)의 λ 초기값을 만든다. 경사하강이
개입하지 않으므로 같은 데이터면 항상 같은 결과가 나오고, run마다 재계산해도 무방하다.

## 원리

λ축 fringe 정렬은 경사하강에 비볼록이라 초기값이 틀리면 잘못된 fringe 차수 분지에
안착한다. 대신 타깃이 30⁴ **전수 격자**인 점을 쓴다:

1. 층 j로 조건화한 평균 `g_c(d) = E[R_c | d_j = d]` (30점)를 만들면, 다른 층의 변동이
   주변화로 지워지고 이 곡선은 두께축에서 기본 주파수
   **f_j(c) = 2·n_j(λ_c)/λ_c** [cycles/nm] 로 진동한다 (특성행렬 M_j가 δ→δ+π에서
   부호만 바뀌므로 R은 δ_j에 π 주기).
2. SiO₂ 층(2·4)은 게이지 고정으로 n(λ)를 아니까 `2·n_SiO₂(λ)/λ = f` 를 λ에 대해
   풀면 된다. **g(λ) = 2n/λ 가 단조 감소**하므로 이분법으로 유일해가 나온다.
3. SiN 층(1·3)은 같은 관계에서 n_SiN(λ_c) = f·λ_c/2 추정을 준다.

경사하강이 전혀 개입하지 않으므로 **같은 입력이면 항상 같은 출력**이다. λ축 fringe
정렬을 경사하강에 맡기면 잘못된 fringe 차수 분지에 안착한다 (초기 λ 범위 후보 16개를
짧게 피팅해 보는 방식은 전부 RMSE ~0.12에서 정체했다 — docs/week_1.md).

## 자체 검증

같은 물리량을 서로 다른 층에서 **독립적으로** 측정하므로 그 일치도가 검증이 된다:
층 2 vs 층 4의 λ 추정, 층 1 vs 층 3의 n_SiN 추정, 그리고 n_SiN의 문헌 근접성이
전부 `diagnostics`로 나온다 (`lam24_dev_*`, `n_sin13_dev_*`).

**주의**: 반환된 채널별 λ에는 ~0.44 nm의 흔들림이 있다. λ(c)의 매끈한 구조가 아니며
(다항 차수를 올려도 줄지 않는다), 정체는 **결정론적 추정량 편향**이다 — 주파수 후보 격자
양자화(`n_candidates`)와 6항 기저 절단. 노이즈를 0으로 둔 합성 전수격자에서도 같은 크기가
나온다. 그대로 고정하면 그것만으로 R 오차 rms 0.0052가 생기므로, 본 피팅은 이 결과에
매끈한 3계수 곡선을 적합해 쓴다 (`calibrate.fit_lam_coefficients`).
"""
...
def identify_wavelength_grid(
    ...
    *,
    ...
    # 신뢰도 마스크: 층 2·4의 독립 λ 추정이 어긋나거나 n_SiN이 물리 하한 미만인 채널은
    # 주파수 추정 실패로 보고 이웃 신뢰 채널에서 선형 보간한다. λ가 채널축에서 매끈하다는
    # 물리 가정(분광기 격자)이 근거다. 실데이터에서는 Si E1 임계점 부근 fringe 대비
    # 저하로 소수 채널이 크게 튄다 — 그대로 두면 그 채널의 재구성 오차가 σ의 10배를 넘는다.
    # λ 불일치만으로 판정하면 SiN 주파수 실패를 놓친다 (보간 후에도 물리적으로 불가능한
    # n_SiN ≈ 1.54가 남는다) — 그래서 n_SiN 물리 하한도 함께 본다.
