...
실측 기준점 (2026-08-20): d2-se raw p90 2.3배 → test +85% 붕괴 / d2-fft raw p90 1.95배 →
+65% / d2-fft+LM 파이프라인 p99.9까지 일치 → −0.02%.

**검출 범위 주의 — 통과가 무열화를 보장하지 않는다.** 213M raw는 꼬리 1.03/1.00으로
통과했는데 test에서 +20% 열화했다: 재구성 잔차는 노이즈 바닥(σ 0.0087) 아래의 sub-nm
보간 오차에 둔감하다 (감도 ~0.01/nm). 이 스크립트는 격자 끌림 **붕괴**의 중단 신호이고,
raw 제출 자체를 피하는 것이 규칙이다 (reports/task8.md 발견 ③).
