...
학습 없이 체크포인트 위에서 돈다 — random split MAE 하나로는 보이지 않는 차이를 재는 축이다
(전수 격자라 조합 보간은 이미 쉽다).

- **노이즈 강건성**: 주입 노이즈에 따른 MAE 열화. 데이터에 이미 σ = 0.008658이 있으므로
  주입은 **추가분**이다. 실현을 수준마다 고정해 모든 run이 같은 입력을 본다.
- **신뢰도 지표**: 행별 물리 잔차 L1(R_dec(d̂), R_obs)와 실제 오차의 순위상관. 라벨을
  쓰지 않으므로 test·실계측에도 적용된다.
