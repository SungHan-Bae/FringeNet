...
- **반사율은 [0,1]을 벗어난다 — 데이터에 가산 노이즈가 있다 (중요)**:
  ...
  - 음의 반사율이 train 값의 0.35%, **행의 46.9%**에 등장. 물리적으로 불가능하므로
    참 스펙트럼 위에 노이즈가 얹힌 것.
  - 노이즈 크기 추정 두 가지가 일치 → **σ ≈ 0.0087**:
    2차 차분 추정 0.008722, 균등분포 ±0.0151 가정 시 a/√3 = 0.008728.
...
- Level 2:
  ...
  - 판정 게이트: 재구성 RMSE < 5e-3 (R 단위) → 물리 디코더 채택.
    ...
    - **⚠ 게이트 재설정 필요 (Task 2 검증 결과, 사용자 판단 대기)**: 관측 R에 σ ≈ 0.0087의
      노이즈가 있으므로, **완벽한 forward 모델이라도 관측값 대비 RMSE는 σ 아래로 내려갈 수 없다.**
      5e-3 게이트는 원리적으로 통과 불가이며, 이대로 두면 맞는 TMM을 틀렸다고 기각하게 된다.
      대안: (a) 게이트를 노이즈 바닥 기준으로 재정의 — RMSE < 1.2σ ≈ 0.0105,
      (b) 관측 R을 채널축으로 스무딩해 노이즈를 줄인 뒤 비교, (c) 잔차가 두께에 대해
      구조를 갖는지(=모델 오차) 아니면 백색인지(=노이즈)를 보는 진단으로 대체.
      Task 6 착수 전에 결정할 것.
...
- [x] **Task 2 — 데이터 검증·로더**: `scripts/verify_data.py`(가설 체크 출력),
  ...
  (가설 중 "반사율 ⊂ [0,1]"만 거짓 — σ ≈ 0.0087 가산 노이즈 발견. Stage A 게이트 재설정 필요.)
