...
- **반사율은 [0,1]을 벗어난다 — 데이터에 가산 노이즈가 있다 (중요)**:
  ...
  - **노이즈는 유계다 — |ε| ≤ 0.0152 (중요, 2026-08-12)**. 근거 둘:
    (1) 균등분포면 a = σ√3 = **0.014997 ≈ 0.015** (생성기 설정값으로 보인다),
...
- [x] **Task 6 — Stage A 캘리브레이션**: 게이트 판정까지. **TMM 물리 디코더 채택.**
  ...
  λ = 284~792 nm 내림차순) → λ 3계수 매끈 곡선 적합 → 물리 제약 LM 공동 피팅.
  - **자유 파라미터 7개로 RMSE 0.009941 (1.148σ), 게이트 (a) 통과.** 물성 전부 문헌
    정합, 독립 두 방법(주파수 식별 vs TMM 최소제곱)이 0.3~0.9% 일치,
    **홀드아웃 20채널 예측 RMSE가 피팅 채널의 1.032배**, 두께 nm 역해 MAE **0.388 nm**.
