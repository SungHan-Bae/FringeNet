...
- **노이즈 σ ≈ 0.0088** (독립 추정 3개 일치: 0.008722 / 0.008728 / 0.008867).
  초과 첨도 −0.479 → **균등분포에 가깝다**(균등 −0.6, 가우시안 0).
...
- **층별 민감도** (한 층만 +10 nm, RMS ΔR / SNR): layer_1 0.1105/12.7, layer_2 0.0896/**10.3**,
  ...
  layer_4는 40~60 nm에서 민감도가 0.076까지 꺼지는 국소 최소가 있다(구간별 오차 분석 시 확인).
- **fringe 주기 분포** (지배 주기, 채널 단위): p5 28.2 / 중앙 56.5 / p95 226.0.
  → **1D CNN 수용영역이 28채널 주기부터 대역 전체(226)까지 커버해야 한다.**
  작은 커널만 쌓으면 부족 — dilated conv 또는 커널 혼합이 필수이고, GAP 직전 유효
  수용영역이 226에 도달하는지 확인할 것. 이것이 "다중 스케일"의 정량 요건이다.
- **채널별 정보량 불균등**: mean |ΔR|이 채널 0~9 대비 216~225에서 약 **3배**(2.86~3.10×).
  노이즈는 채널에 균일 → 대역 오른쪽 끝 SNR이 3배 좋다. 채널 자르기·downsample 주의.
  물리 손실의 채널 가중은 ablation 후보(기본은 균등 가중).
- **SiN:SiO₂ 무늬 개수 비 ≈ 2** (300 nm에서 4 vs 2 cycles). 문헌 굴절률 비는 1.37.
  → Stage A 캘리브레이션이 굴절률을 제대로 찾으면 이 비가 정량 재현되어야 한다. sanity check로 쓸 것.
- 죽은 채널 없음(채널별 표준편차 0.1816~0.2124로 고르다). 최대 R = 0.944로 **위쪽 절단 없음**.
- **하지 말 것**: "무늬 개수 ∝ 두께"를 R² 같은 정량 법칙으로 주장하기. 지표 3종을 시도했으나
  파장축 비식별화(채널 간격 불명) + 대역에 무늬 1~8개뿐 + 4층 beat + 노이즈 때문에
  신뢰할 수준이 안 나온다. 특히 네 층 모두 10 nm면 스펙트럼이 거의 평평해 극값 세기
  지표는 노이즈를 세고 **부호가 반대로** 나온다.
