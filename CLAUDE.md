...
- Level 2:
  - **Stage A = `src/calibrate.py` (물리 제약 LM 피팅, 자유 파라미터 1~7개).**
    ...
    - Si 기판: **Aspnes & Studna 1983 원본 실측표 + 에너지축 3차 스플라인** 동결
      ...
      **Si 표는 Schinke 2015를 쓴다 (2026-08-13 측정으로 확정)**. 기존 논거 "Aspnes만
      ...
      **세 표의 상호 불일치가 유계 노이즈 예산의 69%**(맨 Si R 차이 최대 0.0105 = 1.22σ,
      4층 스택 0.0185)이므로 게이트 (f) 불확실성에 이 항을 반드시 함께 적는다.
      문헌 원본은 `src/physics/literature/*.yml` (CC0, git 추적).
...
- [x] **Task 6 — Stage A 캘리브레이션**: 게이트 판정까지. **TMM 물리 디코더 채택.**
  ...
  - **자유 파라미터 7개 + Si 표 Schinke 2015로 RMSE 0.009573 (1.106σ), 게이트 (a) 통과.** 물성 전부 문헌
    ...
    **채널 홀드아웃 예측 성립** (단파장 끝 연속블록 ch206–225의 한계 효과 **+14.0%** —
  ...
  - **미해결: 게이트 (b) 유계 노이즈 위반 9.99% — 미통과.** 남은 오차는 **c-Si 임계점
