...
- Level 2:
  - **Stage A = `src/calibrate.py` (물리 제약 LM 피팅, 자유 파라미터 1~7개).**
    ...
    - Si 기판: **Aspnes & Studna 1983 원본 실측표 + 에너지축 3차 스플라인** 동결
      ...
      이고 c-Si는 E1(3.40 eV)·E2(4.25 eV) 임계점 구조가 지배한다. Aspnes를 쓰는 이유는
      0.1 eV 균등 격자라 E1에 격자점이 놓이기 때문 (Green 2008은 λ축 10 nm 균등이라
      봉우리를 비껴간다). 문헌 원본은 `src/physics/literature/*.yml` (CC0, git 추적).
