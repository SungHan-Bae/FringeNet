...
- **Task 8 확정 (2026-08-20) — 채택 모델 = `runs/task8/d2-fft` + LM 역산 + 되돌림**:
  holdout **0.3396** / **리더보드 test 0.33895 (4위)**. 백본 = flatten-dilated-bound에
  잔차(`residual: true`, MLPBlock.skip 규약의 conv 판) + 깊이 ×2(블록 10, stride-1 복제) +
  rFFT 입력 분기(`fft_channels`) — 1.52M 파라미터. 정본은 `reports/task8.md`(서사) ·
  `task8_judge.md`(판정 표) · `task8_bench.md`(추론 비용). 확정 사실 다섯:
  - **병목=적합력 진단 확증, 레버는 잔차+깊이**: train_l1 벽 1.4754 → 0.30. 깊이 ≫ 폭
    (d2 1.5M이 w2 2.6M을 val 3배 차로 이김) — 작은 모델이 버는 방향은 깊이다.
  - **물리 보정의 교차점**: LM 이득이 CNN 품질에 따라 붕괴(−74% → −6.4% → d2-se에서
  ...
  - **제출 전 관문 (라벨 없음, 검출 범위 주의)**: 재구성 잔차의 holdout↔test 꼬리 비교
    (`check_submission_transfer.py`)는 격자 끌림 **붕괴**를 잡는다 (신호↔열화 단조:
    1.95배→+65%, 2.3배→+85%). 단 **통과 ≠ 무열화** — 213M은 1.03배 통과인데 +20%
    (잔차는 노이즈 바닥 아래 sub-nm 오차에 둔감). raw 제출 자체를 하지 않는 것이 규칙.
  - SE 어텐션(d2-se)은 CNN val 최강이지만 post-LM에 남지 않고 격자 흡수만 커진다 —
    채택은 fft 가지. 결합(d2-se-fft)은 동률(이득 겹침).
  - 추론 배포 경로(c64 해석적+조기종료) 합계 = skip-MLP 대비 CPU 1.42배·L4 1.49배
    (기계 명시 규약 유지 — d2-fft forward 단독은 213M보다 빠르다).
- **역산 refinement = `src/physics/invert.py` + `scripts/refine_inversion.py`** (Task 7의
  ...
  - **되돌림 규칙 = `flag_unreliable` (2026-08-17 확정).** 재구성 잔차가 **중앙값 + 5·robust σ**
    ...
    - **확정**: `cnn_recipe/budget100` 기준 holdout 81,000행에서 **0.4884 → 0.3880 nm**.
      ...
      표본 오차가 없다. 정본은 `reports/cnn_recipe_judge.md`.
    - **격자 밖 확인 완료 — 리더보드 test MAE 0.38733** (데이콘 제출 2026-08-18, 15위).
      holdout 0.3880이 격자 밖 연속 두께에서 열화 없이 전이된다 (−0.2%) — 우려했던 잔차
      극단 꼬리 2배는 MAE에 반영되지 않았다. 단 **213M과의 격자 밖 직접 비교는 미측정**이다
      (skip-MLP 예측은 제출하지 않았다 — holdout 비교 0.3880 vs 0.3955만 확정).
...
- [x] **Task 7 — Stage B**: 종결 — β 기각 · 역산 refinement · 되돌림 규칙 · 리더보드 확인까지
  ...
  - [x] **리더보드 업로드 — test MAE 0.38733 (2026-08-18, 15위).** holdout 0.3880이 격자 밖에서
        열화 없이 전이 (위 되돌림 스펙). 파일은
...
- [x] **Task 8 — 모델·학습 최적화: 종결.** 구조(잔차) → 용량(깊이 ×2) → 부착 모듈(rFFT)로
      **채택 = `runs/task8/d2-fft` + LM + 되돌림, holdout 0.3396 / 리더보드 test 0.33895
      (4위, 2026-08-20)**. 핵심 발견은 물리 보정의 교차점과 격자 밖 반전 (위 Task 8 스펙,
      정본 `reports/task8.md`). 라운드 5개 완료 (`notebooks/task8/`), 자원 미터 갱신
      (322배 → 140배).
- [x] **Task 9 — 문서화: 종결.** 루트 README = **슬림 쇼케이스** (취지 → 접근 3항 →
      결과: 3자 비교 holdout/test 두 열 + 격자 밖 반전 + 자원 미터 + 한계 → 구조 →
      시작하기, 약 130줄). 방법 상세는 넣지 않는다 — 전부 reports/* 링크로
      (심사자 가독성이 우선, 상세의 정본은 리포트다). `reports/README.md`도 표 3개
      (진행 비교·읽기 순서·파일 구분)만 남긴 인덱스로 유지. task8 반영,
...
- **raw 신경망 예측을 제출하기** — 측정한 raw 모델 셋 전부(우리 둘 + 213M) test에서
  +20~85% 열화했고 물리 파이프라인만 전이됐다 (Task 8 스펙). 제출은 항상
  `--submission --refine` 경로. `scripts/check_submission_transfer.py` 관문은 격자 끌림
  붕괴의 중단 신호일 뿐 **통과가 무열화를 보장하지 않는다** (213M: 통과인데 +20%)
