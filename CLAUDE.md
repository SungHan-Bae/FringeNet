...
- Level 2:
  - **Stage A = `src/calibrate.py` (물리 제약 최소제곱 피팅, 자유 파라미터 1~7개).**
    ...
    - λ(c): `1/λ = ν₀(1 + r₁u + r₂u²)`, u = c/(W−1) — 분광기 격자 분산은 매끈·단조.
      ...
      **채널별 226개를 그대로 고정하지 말 것** — 그 값에는 0.445 nm rms 추정 잡음이
      있고(다항 차수를 올려도 줄지 않는다 — 주파수 후보 격자 양자화·기저 절단에서
      오는 결정론적 편향이다) 그것만으로 R 오차 rms 0.0052가 생긴다 (최종 모델에 남은
      계통오차 전체 0.0041보다 크다).
    ...
    - Si 기판: **Schinke 2015 원본 실측표 + 에너지축 3차 스플라인** 동결
      (선택적으로 ΔE 시프트·k 스케일 2개). 코드 기본값도 이것이다
      (`src.calibrate.DEFAULT_SI_SOURCE` — 기본값을 기각된 표로 두면 `si_source`를
      빠뜨린 config가 조용히 그쪽으로 돈다). **Tauc-Lorentz 쓰지 말 것** — TL은 비정질용
      이고 c-Si는 E1(3.40 eV)·E2(4.25 eV) 임계점 구조가 지배한다 (TL이 맞는 자리는
      비정질 SiN의 UV 흡수 쪽이다).
      **표를 Schinke로 정한 것은 2026-08-13 측정이다.** 기존 논거 "Aspnes만
      0.1 eV 균등 격자라 E1에 격자점이 놓인다(Green/Schinke는 비껴간다)"는 반증됐다 —
      에너지축 3차 스플라인이 10 nm λ 격자에서도 봉우리를 복원해 세 표의 E1 위치가
      3.5 meV 이내로 일치한다. `si_source`만 바꿔 셋을 전부 적합한 결과:
      **Schinke 0.009573 / 9.99% / 0.340 nm > Green 0.009788 / 11.17% / 0.364 >
      Aspnes 0.009941 / 12.01% / 0.388** (RMSE / 위반율 / 역해 MAE). 유지되는 원칙은
      "**해석적 수식이 아니라 실측 원본표 + 에너지축 스플라인**"이다.
      **세 표의 상호 불일치가 유계 노이즈 예산의 70%**(맨 Si R 차이 최대 0.01067 = 1.23σ,
      4층 스택 0.0185)이므로 게이트 (f) 불확실성에 이 항을 반드시 함께 적는다 — 단
      **채택 모델의 최악 채널(ch222–225)에는 이 불일치가 거의 없다**(0.38σ)는 것도 함께
      본다. 문헌 원본은 `src/physics/literature/*.yml` (CC0, git 추적).
  - **게이지 고정 필수**: delta = 2*pi*n*d/lambda 는 (n, lambda)의 공통 스케일에 불변이라
    ...
    물리적으로 무의미한 해에 안착한다). 상관행렬에서 ν₀ ↔ SiN B₁ = −0.686으로 이 축이
  ...
  - **판정 게이트 (2026-08-12 확정, (a)~(f)).** 재구성 잔차만 보는 게이트는 계통오차를
    ...
    - (a) 재구성 RMSE < 1.2σ = **0.010390** (σ = 0.008658). 1.2 배수에 유도는 없으니
      **계통오차 √(RMSE²−σ²)를 함께 1차 지표로** 읽는다 (채택 모델 0.004085).
      원 기준 5e-3은 폐기 —
    ...
    - (b) **유계 노이즈 위반율** (신규, 가장 날카롭다). 노이즈가 |ε| ≤ 0.0152로
      ...
      **어느 물리가 부족한지 국소화**해 준다. 현행 최선이 9.99% — **미통과**.
      **미통과인데 fallback으로 가지 않은 근거와 수용 리스크는 README §3.2 인용 블록에
      명시돼 있다** (역해 0.340 nm ≪ 규제 대상 2.346 nm / 잔차 지배 성분이 문헌표 불일치).
...
- [x] **Task 6 — Stage A 캘리브레이션**: 게이트 판정까지. **TMM 물리 디코더 채택.**
  ...
  식별 λ = 283.8~791.3 nm 내림차순) → λ 3계수 매끈 곡선 적합 → 물리 제약 신뢰영역 공동
  피팅 (최종 λ = 283.4~790.3 nm).
  - **자유 파라미터 7개 + Si 표 Schinke 2015로 RMSE 0.009573 (1.106σ), 게이트 (a) 통과.** 물성 전부 문헌
    정합, 독립 두 방법(주파수 식별 vs TMM 최소제곱)이 0.32~0.74% 일치(채택 모델 0.74%),
    **채널 홀드아웃 예측 성립** (단파장 끝 연속블록 ch206–225, 채택 표 판본의 한계 효과
    **+19.7%**(`holdout-block-uv-schinke`; Aspnes 판본 +14.0%) —
    균등 간격 20채널의 "1.032배"는 대조군 대비 한계 효과가 +0.3%뿐이라 판별력이 없다.
    reports/stage_a.md 게이트 (e) 절), 두께 nm 역해 MAE **0.340 nm**,
    **λ 절대 스케일 검정 통과** (SiO₂ 배율 해방 + Si 동결 → 1.00689, |s−1| = 0.689% < 1%).
  - **Si 실측표 + 에너지축 스플라인이 단일 최대 기여** — 거친 표 대조군 대비 역해 MAE
    1.104 → 0.663 nm. ablation 사다리: 대조군 0.016454 → 표 교체 0.012565 →
    λ 해방 0.011367 → 물성 손잡이 추가 0.009941 → Si 표 Schinke 교체 **0.009573**.
  - **미해결: 게이트 (b) 유계 노이즈 위반 9.99% — 미통과.** 채택을 유지한 근거와
    수용 리스크는 README §3.2 인용 블록에 있다. 잔차 국소화는 **채택 모델 기준**
    (`diagnose_calibration.py` 자동 산출): 임계점 ±0.12 eV 14.2% vs 나머지 8.8%(1.55배),
    **최악 채널은 E1이 아니라 대역 단파장 끝 ch222–225**(26.7%, λ<310 nm = Luke
    유효범위 밖 외삽)이고 **그 구간엔 문헌표 불일치가 거의 없다(0.38σ)** — 표로 설명되지
    않는 모델 부족이다. 후속 후보 순서: ① SiN 단파장 분산 + 층 k=0(Tauc-Lorentz의 자리는
    c-Si가 아니라 비정질 질화막), ② 임계점 파라메트릭 모형(Herzinger 1998 PSEMI) — E1
    구역은 위반율·표 불일치가 함께 크므로 유효, ③ 계면층.
  - **Stage B 디코더 = `runs/stage_a/joint-lam3-sin2-si2-schinke/model.pt`**.
    착수 시 `theta` 7개를 `requires_grad_(False)`로 **명시 동결**할 것 — `nn.Parameter`라
...
python scripts/check_lam_leakage.py                                       # 식별의 판정표본 포함 영향
```

Stage A 경로는 최대 상주 메모리 **5~6 GB**를 쓴다 (조건부 평균이 train 전체를 올린다).
