...
- **Task 8 확정 (2026-08-20) — 채택 모델 = `runs/task8/d2-fft` + LM 역산 + 되돌림**:
  ...
  - **격자 밖 반전 (중요)**: holdout 최강 raw CNN(d2-se 0.2954)이 test에서 **0.5461로
    붕괴** (+85%) — 정확도가 격자 간격보다 좋아지면 raw CNN은 격자 보간을 흡수한다.
    물리 보정의 값어치 = 정밀도가 아니라 **격자 밖 강건성** (파이프라인 전이 −0.02%).
  - **제출 전 관문 (라벨 없음)**: 재구성 잔차의 holdout↔test 분포 비교 — raw d2-se는
    꼬리가 p90 2.3배로 벌어졌고(붕괴 예측), 파이프라인은 p99.9까지 일치했다.
...
- **holdout에서 이긴 raw CNN 예측을 검증 없이 제출하기** — 정확도가 격자 간격보다 좋아지면
  raw CNN은 격자 보간을 흡수해 holdout만 좋아진다 (d2-se 0.2954 → test 0.5461 실사례,
  SE 특유가 아니라 d2-fft raw도 같은 신호). 제출 전 관문:
  `python scripts/check_submission_transfer.py --run <run>` (재구성 잔차 holdout↔test
  꼬리 배수 ≥ 2.0이면 raw 제출 중단 → `--submission --refine` 경로를 쓴다)
