...
1. **Stage A 판정 게이트(§3.2)를 재설정해야 한다.** 관측 R에 σ ≈ 0.0087의 노이즈가 있으면
...
- **판정 게이트:** Stage A 재구성 RMSE < 5×10⁻³ (R 단위)이면 물리 디코더 채택.
  **단 이 기준은 재설정이 필요하다** — §2.2에서 확인한 데이터 노이즈 σ ≈ 0.0087이
  재구성 오차의 하한이므로, 완벽한 forward 모델도 5×10⁻³에 도달할 수 없다.
  노이즈 바닥 기준(예: RMSE < 1.2σ)이나, 잔차가 두께에 대해 구조를 갖는지
  (모델 오차) 백색인지(노이즈)를 보는 진단으로 바꿔야 한다.
...
- ~~시뮬레이션 데이터라 노이즈가 거의 없다.~~ **검증 결과 틀렸다** — §2.2 참조.
  ...
  하한이 생겨 §3.2 게이트를 노이즈 바닥 기준으로 다시 잡아야 한다.
