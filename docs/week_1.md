...
- [ ] **Stage A 게이트 (b) 미통과 후속 — 우선순위 높음.** 남은 오차가 c-Si 임계점
  ...
  31채널의 Luke 유효범위 외삽 처리, ③ SiN UV 흡수(Tauc-Lorentz), ④ 계면층
...
- [ ] **test 격자 밖 사실의 코드 반영 (08-13 리뷰)** — ① `scripts/verify_data.py`에
  "test 두께는 격자 밖" 검정 추가(유계 노이즈 반증), ② `src/evaluate.py --snap` 도움말이
  test에 대해 순손실임을 경고, ③ holdout(격자 위) vs test(격자 밖) 대비를 §3.5-2의
  격자 밖 일반화 측정으로 정식 채택
- [ ] **게이트 (f) 불확실성 표에 Si 표 계통 추가 (08-13 리뷰)** — 표 간 R 차이가
  최대 0.0105(=1.22σ, 유계 상한의 69%)로 지배적인데 SiN 0.3~0.6%만 실려 있다
- [ ] **테스트 커버리지 보강 (08-13 변이 테스트)** — 복소 부호 관례(1j→−1j 8.97σ),
  스플라인 삼중대각 조립(19변이 생존, 최대 6.01σ), L≥3 층 순서, T 경로. 최소 비용안:
  test 2(에너지 보존)에 복소 ns + n0=1.31 케이스 한 줄 추가 → T·n0 변이 4종 동시 사살
- [ ] **freq_id 무작위 부분표집 열화 테스트 (08-13 리뷰)** — 현재 테스트는 15⁴ 전수
  격자 커버리지만 고정. 5만 행에서 ~200/226 채널이 붕괴하는 것을 고정하는 테스트가 없다
- [ ] **`src/train_gpu.py:529↔530` 순서 교정 (08-13 리뷰)** — 미러 사본 train.log의
  마지막 줄 누락. 커밋된 4개 run 로그가 이미 잘려 있다
- [ ] **재현 커맨드의 메모리 요구 명시 (08-13 리뷰)** — 실측 3.9~5.3 GB RSS
