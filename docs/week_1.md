...
- **파라미터가 독립 두 방법과 일치**: SiN B₁ 2.8564(주파수) vs 2.8735(TMM) = 0.9%,
  √C₁ 131.26 vs 131.66 nm = 0.3%. **λ는 주파수 식별 자체 흔들림(0.445)보다 적게
  움직였다(0.368 nm rms)**. 물성 전부 문헌 범위 내 (SiN −5%, Si ΔE −18.7 meV, k ×0.977).
- **상관행렬이 게이지 축을 보여준다**: ν₀ ↔ SiN B₁ = −0.687. B₁ ↔ C₁ = −0.915라 기본은
  B₁ 하나. Si는 λ·SiN과 거의 무상관(|ρ|<0.14 — 위상이 아니라 진폭 정보원).
- **속도**: 자유도가 줄어 LM 피팅이 12~75초 (전날 Adam 피팅은 phase 1+2에 ~1.6시간).

### 미해결 — 게이트 (b)를 통과하는 모델이 없다

최선이 12.0% 위반. 남은 오차는 **c-Si 임계점 E1(3.40 eV)·E2(4.25 eV)에 집중**
(±0.12 eV 38채널 18.7% vs 나머지 188채널 10.7%). 두께축으로는 구조 없음(8.3~8.7%).
...
| Stage A 확정 (Stage B 디코더) | **물리 제약, 자유 파라미터 7개**: RMSE **0.009941 (1.148σ)** ✓(a), 역해 MAE **0.388 nm**, 채널 홀드아웃 1.032배, 물성 전부 문헌 정합. λ 283–790 nm 내림차순. **게이트 (b) 미통과 (위반율 12.0%)** | [reports/stage_a.md](../reports/stage_a.md) |
