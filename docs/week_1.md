...
- [ ] **Si 표 재선정 (08-13 리뷰, 우선순위 높음)** — 측정 결과 Schinke 2015가 네 지표
  전부 Aspnes보다 낫다(RMSE 0.009573 / 위반율 9.99% / 역해 MAE 0.340). `si_source`만 바꾼
  run 2개를 정식 산출물로 커밋하고, 표 선택 근거를 격자 기하학이 아니라 측정으로 다시
  세울 것. Stage B 디코더 확정 전에 끝내야 한다 (run당 ~1분)
