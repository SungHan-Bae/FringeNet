...
| [stage_a.md](../reports/stage_a.md) | **Task 6 Stage A 캘리브레이션** — 물리 제약 최소제곱 피팅, 자유 파라미터 **7개**(Si 표 = Schinke 2015)로 RMSE **0.009573 (1.106σ)**, 두께 역해 MAE **0.340 nm**, **연속블록 채널 홀드아웃 예측 성립**(한계 효과 +19.7%), **λ 절대 스케일 검정 통과**, 물성 전부 문헌 정합. 실측 원본표 + 에너지축 스플라인이 단일 최대 기여. 유계 노이즈 게이트는 미통과(9.99%) — 남은 오차가 **대역 단파장 끝(Luke 유효범위 밖 외삽)** 에 몰리고 그 구간엔 문헌표 불일치가 없다(모델 부족) |
...
| [stage_a_leakage.md](../reports/stage_a_leakage.md) | 주파수 식별이 판정 표본을 2.74% 포함하는 예외의 영향 — `scripts/check_lam_leakage.py` 산출물. λ 해방 run은 6자리 동일(영향 0), λ 동결 run만 1.8% 폭 |
