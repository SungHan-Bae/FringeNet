...
| 노트 | 기간 | 요약 |
|---|---|---|
| [week_1.md](week_1.md) | 2026-08-08 ~ 08-14 | 스캐폴드 → TMM(테스트 7종) → 데이터 검증(**노이즈 σ ≈ 0.0087 발견**) → EDA → Stage A 게이트 확정 → **baseline 4.599 nm** → GPU(Colab) 전환 → CNN ablation 3라운드 (**flatten-dilated-bound 2.346 nm**, −49%) → **1등 단일 모델 재현 0.3955 nm** (상한 기준선) |
...
| Week 2 (08-15~08-21) | 구조 ablation(MLP/CNN, 다중 스케일) · Stage A 캘리브레이션+게이트 판정 · Stage B 물리 손실 학습 | 구조 ablation은 **08-11 완료** (Week 1로 당겨짐) · 나머지 예정 |
...
| 리포트 | 내용 |
|---|---|
| [eda_notes.md](../reports/eda_notes.md) | **Task 3 EDA 관찰·해석** — 노이즈 σ ≈ 0.0087(균등에 가까움) 유도, 층별 민감도 SNR(사각지대 없음), 채널 정보량 3배 |
